<template>
  <a-drawer
    ...
    :title="`岗位分类：${title}（共 ${list.length} 人）`"
    :width="1000"
    class="detail-drawer"
    unmount-on-close
    @cancel="handleClose"
...
<script setup lang="ts">
  ...
  const title = computed(() => props.categoryName);
  ...
  const fetchList = async (): Promise<void> => {
    if (![1, 2, 3].includes(props.jobCategory)) return;
    try {
      loading.value = true;
      const { data } = await dashboardApi.getEmployeesByJobCategory(
        props.jobCategory
      );
      list.value = data ?? [];
      searchKey.value = '';
  ...
  watch(
    () => [visible.value, props.jobCategory] as const,
    ([v]) => {
      if (v && [1, 2, 3].includes(props.jobCategory)) {
        fetchList();
      }
    }
  );
