<template>
  <a-drawer
    ...
    :title="`职称等级：${title}（共 ${list.length} 人）`"
    :width="720"
    class="detail-drawer"
    unmount-on-close
    @cancel="handleClose"
...
<script setup lang="ts">
  ...
  interface Props {
    modelValue: boolean;
    /** 职称等级名称：初级/中级/副高级/正高级 */
    titleLevelName: string;
    /** 职称等级值：1-初级，2-中级，3-副高级，4-正高级 */
    titleLevel: number;
  }
  ...
  const title = computed(() => props.titleLevelName);
  ...
  const fetchList = async (): Promise<void> => {
    if (![1, 2, 3, 4].includes(props.titleLevel)) return;
    try {
      loading.value = true;
      const { data } = await dashboardApi.getEmployeesByTitleLevel(
        props.titleLevel
      );
      list.value = data ?? [];
      searchKey.value = '';
  ...
  watch(
    () => [visible.value, props.titleLevel] as const,
    ([v]) => {
      if (v && [1, 2, 3, 4].includes(props.titleLevel)) {
        fetchList();
      }
    }
  );
