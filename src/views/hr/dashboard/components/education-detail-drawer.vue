<template>
  <a-drawer
    ...
    :title="`学历：${title}（共 ${list.length} 人）`"
    :width="1000"
    class="detail-drawer"
    unmount-on-close
    @cancel="handleClose"
...
<script setup lang="ts">
  ...
  const props = withDefaults(defineProps<Props>(), { degree: 0 });
  ...
  const title = computed(() => props.degreeName);
  ...
  const fetchList = async (): Promise<void> => {
    if (![0, 2, 3, 4, 5].includes(props.degree)) return;
    try {
      loading.value = true;
      const { data } = await dashboardApi.getEmployeesByEducation(props.degree);
      list.value = data ?? [];
      searchKey.value = '';
  ...
  watch(
    () => [visible.value, props.degree] as const,
    ([v]) => {
      if (v && [0, 2, 3, 4, 5].includes(props.degree)) fetchList();
    }
  );
</script>
