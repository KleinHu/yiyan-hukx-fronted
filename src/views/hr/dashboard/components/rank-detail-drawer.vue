<template>
  <a-drawer
    ...
    :title="`职级分布：${title}（共 ${list.length} 人）`"
    :width="1000"
    class="detail-drawer"
    unmount-on-close
    @cancel="handleClose"
...
<script setup lang="ts">
  ...
  const title = computed(() => props.rankName);
  ...
  const fetchList = async (): Promise<void> => {
    if (!props.rankName) return;
    try {
      loading.value = true;
      const { data } = await dashboardApi.getEmployeesByRank(props.rankName);
      list.value = data ?? [];
      searchKey.value = '';
  ...
  watch(
    () => [visible.value, props.rankName] as const,
    ([v]) => {
      if (v && props.rankName) {
        fetchList();
      }
